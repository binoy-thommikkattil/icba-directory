            ...
            allFamilies.forEach(family => {
                ...
                family.members?.forEach((member: any) => {
                    const hasTag = member.tags?.some((t: string) => {
                        ...
                        return tag.includes('youth') || tag.includes('young family');
                    });

                    if (hasTag) {
                        matches.push({ familyId: family.id, familyName: family.familyName, name: member.name, fullFamilyData: { id: family.id, ...family } });
                    }
                });
            });

            const uniqueMatches = matches.filter((ind, index, self) => index === self.findIndex((m) => m.name.toLowerCase() === ind.name.toLowerCase()));
