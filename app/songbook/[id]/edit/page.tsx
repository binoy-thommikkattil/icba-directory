  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(docRef, {
        ...
        language,
        lyrics,
        transliterationEnglish: transliteration,
        meaningEnglish: meaningEng,
        meaningMalayalam: meaningMal,
        story,
        authorName, // Stamp the person who edited it last
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 pb-24">
      <div className="max-w-2xl mx-auto">
        <Link href={`/songbook/${params.id}`} className="mb-6 inline-flex items-center text-sm font-bold text-slate-500 hover:text-slate-800 transition">
          <ArrowLeft size={16} className="mr-1" /> Back to Song
        </Link>

        <h1 className="text-3xl font-serif font-bold text-slate-900 mb-2">Edit Song</h1>
        <p className="text-slate-500 text-sm mb-8">Update the lyrics, add manual transliteration, or fix the meanings.</p>
        ...
        <form onSubmit={handleUpdate} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2 sm:col-span-1">
              ...
              <input required type="number" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-sky-500 transition" value={songNumber} onChange={e => setSongNumber(e.target.value)} />
            </div>
            <div className="col-span-2 sm:col-span-1">
          ...
          {language !== 'English' && (
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex justify-between">
                ...
                <span className="text-sky-500 font-medium">For singing along</span>
              </label>
              <textarea rows={6} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-sky-500 transition leading-relaxed" value={transliteration} onChange={e => setTransliteration(e.target.value)} placeholder="Type the phonetic English here..." />
            </div>
          )}
          ...
          <div className="pt-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <textarea rows={4} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-sky-500 transition text-sm" value={meaningEng} onChange={e => setMeaningEng(e.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Malayalam Meaning</label>
              <textarea rows={4} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-sky-500 transition text-sm" value={meaningMal} onChange={e => setMeaningMal(e.target.value)} />
            </div>
          </div>
